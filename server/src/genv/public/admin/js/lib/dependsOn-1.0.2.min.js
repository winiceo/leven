...
(function ($) {
  var Dependency = function (selector, qualifiers) {
    this.selector = selector;this.$dependencyObj = $(selector);this.qualifiers = qualifiers;
    ...
    var dependencyValue = this.$dependencyObj.val(),
        ...
        i = 0,
        match = false;if (this.$dependencyObj.attr("type") === "radio") {
    ...
    var dependencyValue = this.$dependencyObj.val(),
        length = checkAgainst.length,
        i = 0;if (this.$dependencyObj.attr("type") === "radio") {
      dependencyValue = this.$dependencyObj.filter(":checked").val();
    }for (i; i < length; i += 1) {
      if (checkAgainst[i] === dependencyValue) {
        return false;
    ...
    var dependencyValue = this.$dependencyObj.val(),
        pattern = checkAgainst;return pattern.test(dependencyValue);
  };Dependency.prototype.contains = function (checkAgainst) {
    var dependencyValue = this.$dependencyObj.val(),
        i = 0;if (typeof dependencyValue === "array" || typeof dependencyValue === "object") {
    ...
    var emailPattern = /^[_a-zA-Z0-9\-]+(\.[_a-zA-Z0-9\-]+)*@[a-zA-Z0-9\-]+(\.[a-zA-Z0-9\-]+)*\.(([0-9]{1,3})|([a-zA-Z]{2,3})|(aero|coop|info|museum|name))$/;return this.match(emailPattern) === checkAgainst;
  };Dependency.prototype.url = function (checkAgainst) {
    var urlPattern = /(((http|ftp|https):\/\/)|www\.)[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?\^=%&:\/~\+#!]*[\w\-\@?\^=%&\/~\+#])?/g;return this.match(urlPattern) === checkAgainst;
  };Dependency.prototype.doesQualify = function () {
    var q = 0;for (q in this.qualifiers) {
    ...
    var d = 0;this.dependencies = [];for (d in dependencies) {
    ...
    var length = this.dependencies.length,
        d = 0,
        qualifies = true;for (d; d < length; d += 1) {
      if (!this.dependencies[d].doesQualify()) {
        qualifies = false;break;
      }
    }return qualifies;
  };var DependencyCollection = function ($subject, initalSet, options) {
    this.dependencySets = [];this.$subject = $subject;this.settings = $.extend({ disable: true, hide: true, duration: 200, onEnable: function () {}, onDisable: function () {} }, options);this.enableCallback = function () {};this.disableCallback = function () {};this.init(initalSet);
  };DependencyCollection.prototype.init = function (dependencies) {
    this.addSet(dependencies);this.check(true);
  };DependencyCollection.prototype.addSet = function (set) {
    var self = this,
        thisSet = 0,
        numDependencies = 0,
        d = 0,
        dependency;this.dependencySets.push(new DependencySet(set));thisSet = this.dependencySets.length - 1;numDependencies = this.dependencySets[thisSet].dependencies.length;for (d; d < numDependencies; d += 1) {
      dependency = this.dependencySets[thisSet].dependencies[d];dependency.$dependencyObj.on("change", function (e) {
        self.triggeredEvent = e;self.triggeredDependency = this;self.check();
    ...
    this.addSet(dependencies);this.check(false);return this;
  };DependencyCollection.prototype.enable = function (noFade) {
    var valueSubject = this.$subject,
        subjectId = this.$subject.attr("id"),
        $hideObject;if (this.settings.hasOwnProperty("valueTarget") && this.settings.valueTarget !== undefined) {
    ...
    var length = this.dependencySets.length,
        i = 0,
        qualifies = false;for (i; i < length; i += 1) {
      if (this.dependencySets[i].doesQualify()) {
        qualifies = true;break;
      }
    }if (qualifies) {
      this.enable(firstRun);return true;
    } else {
      this.disable(firstRun);return false;
    }
  };$.fn.dependsOn = function (dependencies, options) {
    var dependencyCollection = new DependencyCollection(this, dependencies, options);return dependencyCollection;
  };
})(jQuery);
//# sourceMappingURL=dependsOn-1.0.2.min.js.map
