  ...
  */
	Dependency.prototype.checked = function (checkAgainst) {
		...
		if (this.$dependencyObj.attr('type') === 'checkbox') {
			...
			if (!this.$dependencyObj.is(':checked') && checkAgainst || this.$dependencyObj.is(':checked') && !checkAgainst) {
	...
	Dependency.prototype.values = function (checkAgainst) {
		...
		for (i; i < length; i += 1) {
			if (typeof dependencyValue === 'array' || typeof dependencyValue === 'object') {
				...
				if ($(this.$dependencyObj.val()).not($(checkAgainst[i])).length === 0 && $(checkAgainst[i]).not($(this.$dependencyObj.val())).length === 0) {
					match = true;
					break;
				}
			} else {
  ...
  */
	Dependency.prototype.doesQualify = function () {
		...
		for (q in this.qualifiers) {
			...
			// execute it.
			if (Dependency.prototype.hasOwnProperty(q) && typeof Dependency.prototype[q] === 'function') {
...
//# sourceMappingURL=dependsOn-1.0.2.js.map
