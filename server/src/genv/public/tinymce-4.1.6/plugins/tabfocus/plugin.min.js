tinymce.PluginManager.add("tabfocus", function (a) {
  function b(a) {
    ...
    function c(c) {
      ...
      } else for (j = g - 1; j >= 0; j--) if (i(h[j])) return h[j];return null;
    }var g, h, i, j;if (!(9 !== b.keyCode || b.ctrlKey || b.altKey || b.metaKey || b.isDefaultPrevented()) && (i = f(a.getParam("tab_focus", a.getParam("tabfocus_elements", ":prev,:next"))), 1 == i.length && (i[1] = i[0], i[0] = ":prev"), h = b.shiftKey ? ":prev" == i[0] ? c(-1) : d.get(i[0]) : ":next" == i[1] ? c(1) : d.get(i[1]))) {
      var k = tinymce.get(h.id || h.name);h.id && k ? k.focus() : window.setTimeout(function () {
        tinymce.Env.webkit || window.focus(), h.focus();
      }, 10), b.preventDefault();
    }
  }var d = tinymce.DOM,
      e = tinymce.each,
      f = tinymce.explode;a.on("init", function () {
