...
module.exports = function (app) {
    ...
    _.each(_conf, function (value, key) {
        ...
        if (!value.foursquare || !value.foursquare.enable) return;
        ...
        passport.use(new strategy({
            ...
            new _schema('system.apps').init(app).get({
                ...
                new _schema('system.accounts').init(app).get({
                    ...
                    if (err) {
                        if (err.name != 'NotFound') return done(null);
                    }

                    if (!req.session.social) req.session.social = {};
                    ...
                    if (profile.firstName) {
                        ...
                        if (profile.lastName) displayName += ' ' + profile.lastName;
                    }

                    // set profile photo
                    var profilePhoto = '';

                    if (profile.photo && profile.photo.prefix && profile.photo.suffix) profilePhoto = profile.photo.prefix + 'original' + profile.photo.suffix;
