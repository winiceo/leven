 ...
 */
(($ => {$.color={};$.color.make=(r, g, b, a) => {const o={};o.r=r||0;o.g=g||0;o.b=b||0;o.a=a!=null?a:1;o.add=(c, d) => {for(let i=0;i<c.length;++i)o[c.charAt(i)]+=d;return o.normalize()};o.scale=(c, f) => {for(let i=0;i<c.length;++i)o[c.charAt(i)]*=f;return o.normalize()};o.toString=() => {if(o.a>=1){return"rgb("+[o.r,o.g,o.b].join(",")+")"}else{return"rgba("+[o.r,o.g,o.b,o.a].join(",")+")"}};o.normalize=() => {function clamp(min,value,max){return value<min?min:value>max?max:value}o.r=clamp(0,parseInt(o.r),255);o.g=clamp(0,parseInt(o.g),255);o.b=clamp(0,parseInt(o.b),255);o.a=clamp(0,o.a,1);return o};o.clone=() => $.color.make(o.r,o.b,o.g,o.a);return o.normalize()};$.color.extract=(elem, css) => {let c;do{c=elem.css(css).toLowerCase();if(c!=""&&c!="transparent")break;elem=elem.parent()}while(elem.length&&!$.nodeName(elem.get(0),"body"));if(c=="rgba(0, 0, 0, 0)")c="transparent";return $.color.parse(c)};$.color.parse=str => {
    let res;
    const m=$.color.make;
    if(res=/rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(str))return m(parseInt(res[1],10),parseInt(res[2],10),parseInt(res[3],10));if(res=/rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(str))return m(parseInt(res[1],10),parseInt(res[2],10),parseInt(res[3],10),parseFloat(res[4]));if(res=/rgb\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*\)/.exec(str))return m(parseFloat(res[1])*2.55,parseFloat(res[2])*2.55,parseFloat(res[3])*2.55);if(res=/rgba\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(str))return m(parseFloat(res[1])*2.55,parseFloat(res[2])*2.55,parseFloat(res[3])*2.55,parseFloat(res[4]));if(res=/#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/.exec(str))return m(parseInt(res[1],16),parseInt(res[2],16),parseInt(res[3],16));if(res=/#([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])/.exec(str))return m(parseInt(res[1]+res[1],16),parseInt(res[2]+res[2],16),parseInt(res[3]+res[3],16));const name=$.trim(str).toLowerCase();if(name=="transparent")return m(255,255,255,0);else{res=lookupColors[name]||[0,0,0];return m(res[0],res[1],res[2])}
...
(($ => {
    ...
    class Canvas {
        ...
        resize(width, height) {
            ...
            const element = this.element, context = this.context, pixelRatio = this.pixelRatio;
        ...
        render() {
            ...
            for (const layerKey in cache) {
                if (hasOwnProperty.call(cache, layerKey)) {
                    ...
                    const layer = this.getTextLayer(layerKey), layerCache = cache[layerKey];
        ...
        getTextInfo(layer, text, font, angle, width) {
            ...
            if (info == null) {
                ...
                const element = $("<div></div>").html(text)
                    .css({
                        position: "absolute",
                        'max-width': width,
                        top: -9999
        ...
        addText(layer, x, y, text, font, angle, width, halign, valign) {
            ...
            const info = this.getTextInfo(layer, text, font, angle, width), positions = info.positions;
    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        let // the canvas for the plot itself
        surface = null;

        let // canvas for interactive stuff on top of plot
        overlay = null;

        let // jQuery object that events should be bound to
        eventHolder = null;
        ...
        plot.resize = () => {
        	const width = placeholder.width(), height = placeholder.height();
        ...
        function processData() {
            ...
            for (i = 0; i < series.length; ++i) {
                ...
                let xmin = topSentry, ymin = topSentry, xmax = bottomSentry, ymax = bottomSentry;
        ...
        function adjustLayoutForThingsStickingOut() {
            ...
            let minMargin = options.grid.minBorderMargin, axis, i;
        ...
        function setupGrid() {
            ...
            if (showGrid) {
                ...
                const allocatedAxes = $.grep(axes, axis => axis.show || axis.reserveSpace);
        ...
        function setupTickGeneration(axis) {
            ...
            const // norm is between 1.0 and 10.0
            norm = delta / magn;
            ...
            if (opts.alignTicksWithAxis != null) {
                ...
                if (otherAxis && otherAxis.used && otherAxis != axis) {
                    ...
                    if (!axis.mode && opts.tickDecimals == null) {
                        const extraDec = Math.max(0, -Math.floor(Math.log(axis.delta) / Math.LN10) + 1), ts = axis.tickGenerator(axis);
        ...
        function drawGrid() {
            ...
            if (markings) {
                ...
                for (i = 0; i < markings.length; ++i) {
                    const m = markings[i], xrange = extractRange(m, "x"), yrange = extractRange(m, "y");
                    ...
                    const xequal = xrange.from === xrange.to, yequal = yrange.from === yrange.to;
                    ...
                    if (xequal || yequal) {
                        const lineWidth = m.lineWidth || options.grid.markingsLineWidth, subPixel = lineWidth % 2 ? 0.5 : 0;
                        ctx.beginPath();
        ...
        function drawSeriesLines(series) {
            function plotLine(datapoints, xoffset, yoffset, axisx, axisy) {
                ...
                for (let i = ps; i < points.length; i += ps) {
                    let x1 = points[i - ps], y1 = points[i - ps + 1], x2 = points[i], y2 = points[i + 1];
            ...
            function plotLineArea(datapoints, axisx, axisy) {
                ...
                while (true) {
                    ...
                    let x1 = points[i - ps], y1 = points[i - ps + ypos], x2 = points[i], y2 = points[i + ypos];
            ...
            const lw = series.lines.lineWidth, sw = series.shadowSize;
        ...
        function drawSeriesPoints(series) {
            function plotPoints(datapoints, radius, fillStyle, offset, shadow, axisx, axisy, symbol) {
                const points = datapoints.points, ps = datapoints.pointsize;
        ...
        function drawBar(x, y, b, barLeft, barRight, fillStyleCallback, axisx, axisy, c, horizontal, lineWidth) {
            let left, right, bottom, top, drawLeft, drawRight, drawTop, drawBottom, tmp;
        ...
        function insertLegend() {
            ...
            if (options.legend.sorted) {
                if ($.isFunction(options.legend.sorted)) {
                    ...
                    entries.sort((a, b) => a.label == b.label ? 0 : (
            ...
            const table = '<table style="font-size:smaller;color:' + options.grid.color + '">' + fragments.join("") + '</table>';
            if (options.legend.container != null)
                ...
                const legend = $('<div class="legend">' + table.replace('style="', 'style="position:absolute;' + pos +';') + '</div>').appendTo(placeholder);
        ...
        function findNearbyItem(mouseX, mouseY, seriesFilter) {
            ...
            for (i = series.length - 1; i >= 0; --i) {
                ...
                const // precompute some stuff to make the loop faster
                mx = axisx.c2p(mouseX);
                ...
                if (s.lines.show || s.points.show) {
                    for (j = 0; j < points.length; j += ps) {
                        ...
                        const dx = Math.abs(axisx.p2c(x) - mouseX), dy = Math.abs(axisy.p2c(y) - mouseY), dist = dx * dx + dy * dy; // we save the sqrt
        ...
        function triggerClickHoverEvent(eventname, event, seriesFilter) {
            const offset = eventHolder.offset(), canvasX = event.pageX - offset.left - plotOffset.left, canvasY = event.pageY - offset.top - plotOffset.top, pos = canvasToAxisCoords({ left: canvasX, top: canvasY });
        ...
        function drawPointHighlight(series, point) {
            ...
            const highlightColor = (typeof series.highlightColor === "string") ? series.highlightColor : $.color.parse(series.color).scale('a', 0.5).toString();
        ...
        function drawBarHighlight(series, point) {
            ...
            drawBar(point[0], point[1], point[2] || 0, barLeft, barLeft + series.bars.barWidth,
                    () => fillStyle, series.xaxis, series.yaxis, octx, series.bars.horizontal, series.bars.lineWidth);
        }
