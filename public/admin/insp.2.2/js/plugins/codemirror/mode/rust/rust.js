...
CodeMirror.defineMode("rust", () => {
  const indentUnit = 4, altIndentUnit = 2;
  const valKeywords = {
  ...
  const typeKeywords = (() => {
    const keywords = {"fn": "fn", "block": "fn", "obj": "obj"};
    const atoms = "bool uint int i8 i16 i32 i64 u8 u16 u32 u64 float f32 f64 str char".split(" ");
    for (let i = 0, e = atoms.length; i < e; ++i) keywords[atoms[i]] = "atom";
    return keywords;
  })();
  const operatorChar = /[+\-*&%=<>!?|\.@]/;
  ...
  const cx = {state: null, stream: null, marked: null, cc: null};
  function pass() {
  ...
  function parse(state, stream, style) {
    ...
    while (true) {
      const combinator = cc.length ? cc.pop() : block;
      if (combinator(tcat)) {
  ...
  return {
    ...
    indent: function(state, textAfter) {
      ...
      const firstChar = textAfter && textAfter.charAt(0), lexical = state.lexical, type = lexical.type, closing = firstChar == type;
