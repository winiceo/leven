...
CodeMirror.defineMode("css", (config, parserConfig) => {
  ...
  states.media = (type, stream, state) => {
  ...
  states.media_parens = (type, stream, state) => {
    if (type == ")") return popContext(state);
    if (type == "{" || type == "}") return popAndPass(type, stream, state, 2);
    return states.media(type, stream, state);
  };

  states.font_face_before = (type, stream, state) => {
    if (type == "{")
      return pushContext(state, stream, "font_face");
    return pass(type, stream, state);
  };

  states.font_face = (type, stream, state) => {
    if (type == "}") return popContext(state);
    if (type == "word") {
  ...
  const allWords = mediaTypes_.concat(mediaFeatures_).concat(propertyKeywords_)
