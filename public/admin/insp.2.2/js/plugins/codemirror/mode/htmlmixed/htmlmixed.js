...
CodeMirror.defineMode("htmlmixed", (config, parserConfig) => {
  ...
  const scriptTypes = [], scriptTypesConf = parserConfig && parserConfig.scriptTypes;
  scriptTypes.push({matches: /^(?:text|application)\/(?:x-)?(?:java|ecma)script$|^$/i,
                    mode: CodeMirror.getMode(config, "javascript")});
  if (scriptTypesConf) for (let i = 0; i < scriptTypesConf.length; ++i) {
  ...
  return {
    ...
    copyState: function(state) {
      if (state.localState)
        const local = CodeMirror.copyState(state.localMode, state.localState);
