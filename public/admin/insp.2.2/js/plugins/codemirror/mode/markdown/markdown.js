...
CodeMirror.defineMode("markdown", (cmCfg, modeCfg) => {
  ...
  const htmlMode = CodeMirror.getMode(cmCfg, htmlFound ? {name: "xml", htmlMode: true} : "text/plain");
  const aliases = {
    html: "htmlmixed",
    js: "javascript",
  ...
  const header   = 'header', code     = 'comment', quote    = 'quote', list1    = 'variable-2', list2    = 'variable-3', list3    = 'keyword', hr       = 'hr', image    = 'tag', formatting = 'formatting', linkinline = 'link', linkemail = 'link', linktext = 'link', linkhref = 'string', em       = 'em', strong   = 'strong';

  const hrRE = /^([*\-=_])(?:\s*\1){2,}\s*$/,
        ulRE = /^[*\-+]\s+/,
        olRE = /^[0-9]+\.\s+/,
        // Must follow ulRE or olRE
        taskListRE = /^\[(x| )\](?=\s)/,
        atxHeaderRE = /^#+/,
        setextHeaderRE = /^(?:\={1,}|-{1,})$/,
        textRE = /^[^#!\[\]*_\\<>` "'(]+/;
  ...
  const mode = {
    ...
    token: function(stream, state) {
      ...
      if (stream.sol()) {
        const forceBlankLine = !!state.header;
      ...
      const result = state.f(stream, state);
