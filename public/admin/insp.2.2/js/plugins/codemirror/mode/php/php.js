...
((mod => {
  ...
  const phpConfig = {
    ...
    hooks: {
      ...
      "<": function(stream, state) {
        if (stream.match(/<</)) {
          stream.eatWhile(/[\w\.]/);
          const delim = stream.current().slice(3);
          if (delim) {
  ...
  CodeMirror.defineMode("php", (config, parserConfig) => {
    ...
    return {
      startState: function() {
        const html = CodeMirror.startState(htmlMode), php = CodeMirror.startState(phpMode);
        return {html: html,
                php: php,
      ...
      copyState: function(state) {
        ...
        const htmlNew = CodeMirror.copyState(htmlMode, html);
        const php = state.php;
        const phpNew = CodeMirror.copyState(phpMode, php);
        let cur;
