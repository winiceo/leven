...
((mod => {
  ...
  const commonBuiltins = ["abs", "all", "any", "bin", "bool", "bytearray", "callable", "chr",
                        ...
                        "Ellipsis", "__debug__"];
  const py2 = {builtins: ["apply", "basestring", "buffer", "cmp", "coerce", "execfile",
                        "file", "intern", "long", "raw_input", "reduce", "reload",
                        "unichr", "unicode", "xrange", "False", "True", "None"],
             keywords: ["exec", "print"]};
  const py3 = {builtins: ["ascii", "bytes", "exec", "print"],
  ...
  CodeMirror.defineMode("python", (conf, parserConf) => {
    ...
    const singleDelimiters = parserConf.singleDelimiters || new RegExp("^[\\(\\)\\[\\]\\{\\}@,:`=;\\.]");
    const doubleOperators = parserConf.doubleOperators || new RegExp("^((==)|(!=)|(<=)|(>=)|(<>)|(<<)|(>>)|(//)|(\\*\\*))");
    const doubleDelimiters = parserConf.doubleDelimiters || new RegExp("^((\\+=)|(\\-=)|(\\*=)|(%=)|(/=)|(&=)|(\\|=)|(\\^=))");
    const tripleDelimiters = parserConf.tripleDelimiters || new RegExp("^((//=)|(>>=)|(<<=)|(\\*\\*=))");
    ...
    const external = {
      ...
      indent: function(state, textAfter) {
        ...
        const closing = textAfter && textAfter.charAt(0) == scope.type;
        if (scope.align != null)
