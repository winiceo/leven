...
((mod => {
  ...
  class Parser {
    constructor(regExpFactory, state, stream) {
    ...
    handlePhraseModifier(ch) {
      ...
      if (ch === '!') {
        const type = this.togglePhraseModifier('image', /^.*(?:\([^\)]+\))?!/);
    ...
    togglePhraseModifier(phraseModifier, closeRE) {
      if (this.state[phraseModifier]) { // remove phrase modifier
        const type = this.tokenStyles();
    ...
    tokenStyles() {
      const disabled = this.textileDisabled();
      let styles = [];
    ...
    textileDisabled() {
      const type = this.state.layoutType;
    ...
    blankLine() {
      const spanningLayout = this.state.spanningLayout;
      const type = this.state.layoutType;
      let key;
  ...
  CodeMirror.defineMode('textile', () => {
    ...
    return {
      ...
      token: function(stream, state) {
        const parser = new Parser(regExpFactory, state, stream);
