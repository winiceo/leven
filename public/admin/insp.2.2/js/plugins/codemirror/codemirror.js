...
(function(mod) {
  ...
  const gecko = /gecko\/\d/i.test(navigator.userAgent);
  // ie_uptoN means Internet Explorer version N or lower
  const ie_upto10 = /MSIE \d/.test(navigator.userAgent);
  const ie_11up = /Trident\/(?:[7-9]|\d{2,})\..*rv:(\d+)/.exec(navigator.userAgent);
  const ie = ie_upto10 || ie_11up;
  const ie_version = ie && (ie_upto10 ? document.documentMode || 6 : ie_11up[1]);
  let webkit = /WebKit\//.test(navigator.userAgent);
  const qtwebkit = webkit && /Qt\/\d+\.\d+/.test(navigator.userAgent);
  const chrome = /Chrome\//.test(navigator.userAgent);
  let presto = /Opera\//.test(navigator.userAgent);
  const safari = /Apple Computer/.test(navigator.vendor);
  const khtml = /KHTML\//.test(navigator.userAgent);
  const mac_geMountainLion = /Mac OS X 1\d\D([8-9]|\d\d)\D/.test(navigator.userAgent);
  const phantom = /PhantomJS/.test(navigator.userAgent);

  const ios = /AppleWebKit/.test(navigator.userAgent) && /Mobile\/\w+/.test(navigator.userAgent);
  // This is woefully incomplete. Suggestions for alternative methods welcome.
  const mobile = ios || /Android|webOS|BlackBerry|Opera Mini|Opera Mobi|IEMobile/i.test(navigator.userAgent);
  const mac = ios || /Mac/.test(navigator.platform);
  const windows = /win/i.test(navigator.platform);

  let presto_version = presto && navigator.userAgent.match(/Version\/(\d*\.\d*)/);
  ...
  class CodeMirror {
    constructor(place, options) {
      ...
      const display = this.display = new Display(place, doc);
      ...
      for (const opt in optionHandlers) if (optionHandlers.hasOwnProperty(opt))
    ...
    static extendMode(mode, properties) {
    ...
    static defineExtension(name, func) {
      CodeMirror.prototype[name] = func;
    }

    static defineDocExtension(name, func) {
      Doc.prototype[name] = func;
    }

    static defineInitHook(f) {initHooks.push(f);}
  ...
  function Display(place, doc) {
    ...
    const input = d.input = elt("textarea", null, null, "position: absolute; padding: 0; width: 1px; height: 1em; outline: none");
  ...
  function keyMapChanged(cm) {
    const map = keyMap[cm.options.keyMap], style = map.style;
  ...
  function updateGutters(cm) {
    const gutters = cm.display.gutters, specs = cm.options.gutters;
  ...
  function updateScrollbars(cm, measure) {
    ...
    const d = cm.display, sWidth = scrollbarWidth(d.measure);
    const scrollHeight = measure.docHeight + scrollerCutOff;
    let needsH = measure.scrollWidth > measure.clientWidth;
    if (needsH && measure.scrollWidth <= measure.clientWidth + 1 &&
        sWidth > 0 && !measure.hScrollbarTakesSpace)
      needsH = false; // (Issue #2562)
    const needsV = scrollHeight > measure.clientHeight;
  ...
  function visibleLines(display, doc, viewport) {
    ...
    const from = lineAtHeight(doc, top), to = lineAtHeight(doc, bottom);
  ...
  function postUpdateDisplay(cm, update) {
    let force = update.force, viewport = update.viewport;
  ...
  function updateLineGutter(cm, lineView, lineN, dims) {
    ...
    if (cm.options.lineNumbers || markers) {
      const wrap = ensureLineWrapped(lineView);
      const gutterWrap = lineView.gutter =
      ...
      if (markers) for (let k = 0; k < cm.options.gutters.length; ++k) {
        const id = cm.options.gutters[k], found = markers.hasOwnProperty(id) && markers[id];
        if (found)
  ...
  function insertLineWidgetsFor(line, lineView, dims, allowAbove) {
    ...
    for (let i = 0, ws = line.widgets; i < ws.length; ++i) {
      const widget = ws[i], node = elt("div", [widget.node], "CodeMirror-linewidget");
  ...
  const Pos = CodeMirror.Pos = function(line, ch) {
  ...
  const cmp = CodeMirror.cmpPos = (a, b) => a.line - b.line || a.ch - b.ch;
  ...
  function skipAtomicInSelection(doc, sel, bias, mayClear) {
    ...
    for (let i = 0; i < sel.ranges.length; i++) {
      ...
      const newAnchor = skipAtomic(doc, range.anchor, bias, mayClear);
      const newHead = skipAtomic(doc, range.head, bias, mayClear);
  ...
  function skipAtomic(doc, pos, bias, mayClear) {
    let flipped = false, curPos = pos;
    let dir = bias || 1;
    doc.cantEdit = false;
    search: for (;;) {
      const line = getLine(doc, curPos.line);
      if (line.markedSpans) {
  ...
  function drawSelection(cm) {
    const display = cm.display, doc = cm.doc, result = {};
  ...
  function drawSelectionCursor(cm, range, output) {
    const pos = cursorCoords(cm, range.head, "div", null, null, !cm.options.singleCursorHeightPerLine);
  ...
  function drawSelectionRange(cm, range, output) {
    ...
    const padding = paddingH(cm.display), leftSide = padding.left, rightSide = display.lineSpace.offsetWidth - padding.right;
    ...
    function drawForLine(line, fromArg, toArg) {
      ...
      iterateBidiSections(getOrder(lineObj), fromArg || 0, toArg == null ? lineLen : toArg, (from, to, dir) => {
        ...
        if (from == to) {
          ...
          if (dir == "rtl") { const tmp = leftPos; leftPos = rightPos; rightPos = tmp; }
          left = leftPos.left;
  ...
  function highlightWorker(cm) {
    ...
    const state = copyState(doc.mode, getStateBefore(cm, doc.frontier));
    const changedLines = [];

    doc.iter(doc.frontier, Math.min(doc.first + doc.size, cm.display.viewTo + 500), line => {
      if (doc.frontier >= cm.display.viewFrom) { // Visible
        const oldStyles = line.styles;
        const highlighted = highlightLine(cm, line, state, true);
  ...
  function getStateBefore(cm, n, precise) {
    ...
    doc.iter(pos, n, line => {
      processLine(cm, line.text, state);
      const save = pos == n - 1 || pos % 5 == 0 || pos >= display.viewFrom && pos < display.viewTo;
  ...
  function ensureLineHeights(cm, lineView, rect) {
    ...
    const curWidth = wrapping && cm.display.scroller.clientWidth;
  ...
  function measureCharInner(cm, prepared, ch, bias) {
    ...
    const result = {left: (collapse == "right" ? rect.right : rect.left) - prepared.rect.left,
  ...
  function cursorCoords(cm, pos, context, lineObj, preparedMeasure, varHeight) {
    ...
    const partPos = getBidiPartAt(order, ch);
    const val = getBidi(ch, partPos);
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    const innerOff = y - heightAtLine(lineObj);
    let wrongLine = false;
    const adjust = 2 * cm.display.wrapper.clientWidth;
    const preparedMeasure = prepareMeasureForLine(cm, lineObj);

    function getX(ch) {
      const sp = cursorCoords(cm, Pos(lineNo, ch), "line", lineObj, preparedMeasure);
      wrongLine = true;
    ...
    const bidi = getOrder(lineObj);
    let dist = lineObj.text.length;
    ...
    for (;;) {
      if (bidi ? to == from || to == moveVisually(lineObj, from, 1) : to - from <= 1) {
        ...
        const xDiff = x - (ch == from ? fromX : toX);
        while (isExtendingChar(lineObj.text.charAt(ch))) ++ch;
        const pos = PosWithInfo(lineNo, ch, ch == from ? fromOutside : toOutside,
                              xDiff < -1 ? -1 : xDiff > 1 ? 1 : 0);
        return pos;
      }
      const step = Math.ceil(dist / 2);
      let middle = from + step;
      if (bidi) {
        middle = from;
  ...
  function endOperation(cm) {
    const op = cm.curOp, group = op.ownsGroup;
    if (!group) return;
  ...
  function endOperation_finish(op) {
    ...
    if (op.scrollTop != null && (display.scroller.scrollTop != op.scrollTop || op.forceScroll)) {
      const top = Math.max(0, Math.min(display.scroller.scrollHeight - display.scroller.clientHeight, op.scrollTop));
      display.scroller.scrollTop = display.scrollbarV.scrollTop = doc.scrollTop = top;
    }
    if (op.scrollLeft != null && (display.scroller.scrollLeft != op.scrollLeft || op.forceScroll)) {
      const left = Math.max(0, Math.min(display.scroller.scrollWidth - display.scroller.clientWidth, op.scrollLeft));
      display.scroller.scrollLeft = display.scrollbarH.scrollLeft = doc.scrollLeft = left;
      alignHorizontally(cm);
    }
    // If we need to scroll a specific position into view, do so.
    if (op.scrollToPos) {
      const coords = scrollPosIntoView(cm, clipPos(doc, op.scrollToPos.from),
  ...
  function readInput(cm) {
    ...
    let prevInput = cm.display.prevInput;
    const doc = cm.doc;
    ...
    const inserted = text.slice(same), textLines = splitLines(inserted);
    ...
    for (let i = doc.sel.ranges.length - 1; i >= 0; i--) {
      const range = doc.sel.ranges[i];
      ...
      const changeEvent = {from: from, to: to, text: multiPaste ? multiPaste[i % multiPaste.length] : textLines,
      ...
      if (inserted && !cm.state.pasteIncoming && cm.options.electricChars &&
        ...
        const end = changeEnd(changeEvent);
  ...
  function resetInput(cm, typing) {
    ...
    if (cm.somethingSelected()) {
      ...
      const range = doc.sel.primary();
      minimal = hasCopyEvent &&
        (range.to().line - range.from().line > 100 || (selected = cm.getSelection()).length > 1000);
      const content = minimal ? "-" : selected || cm.getSelection();
  ...
  function registerEventHandlers(cm) {
    ...
    on(d.input, "paste", () => {
      ...
      if (webkit && !cm.state.fakedLastChar && !(new Date - cm.state.lastMiddleDown < 200)) {
        const start = d.input.selectionStart, end = d.input.selectionEnd;
        d.input.value += "$";
  ...
  function leftButtonDown(cm, e, start) {
    ...
    const sel = cm.doc.sel, modifier = mac ? e.metaKey : e.ctrlKey;
  ...
  function leftButtonSelect(cm, e, start, type, addNew) {
    ...
    if (e.altKey) {
      ...
      const word = cm.findWordAt(start);
      if (cm.display.shift || doc.extend)
        ourRange = extendRange(doc, ourRange, word.anchor, word.head);
      else
        ourRange = word;
    } else if (type == "triple") {
      const line = new Range(Pos(start.line, 0), clipPos(doc, Pos(start.line + 1, 0)));
      if (cm.display.shift || doc.extend)
    ...
    function extend(e) {
      ...
      const cur = posFromMouse(cm, e, true, type == "rect");
      if (!cur) return;
  ...
  function onDrop(e) {
    ...
    if (files && files.length && window.FileReader && window.File) {
      ...
      var text = Array(n);
      let read = 0;
      const loadFile = (file, i) => {
        const reader = new FileReader;
        reader.onload = operation(cm, () => {
          text[i] = reader.result;
          if (++read == n) {
            pos = clipPos(cm.doc, pos);
            const change = {from: pos, to: pos, text: splitLines(text.join("\n")), origin: "paste"};
      ...
      try {
        ...
        if (text) {
          if (cm.state.draggingText && !(mac ? e.metaKey : e.ctrlKey))
            const selected = cm.listSelections();
  ...
  function allKeyMaps(cm) {
    const maps = cm.state.keyMaps.slice(0);
  ...
  function handleKeyBinding(cm, e) {
    ...
    const startMap = getKeyMap(cm.options.keyMap), next = startMap.auto;
    ...
    const keymaps = allKeyMaps(cm);

    if (e.shiftKey) {
      // First try to resolve full name (including 'Shift-'). Failing
      // that, see if there is a cursor-motion command (starting with
      // 'go') bound to the keyname without 'Shift-'.
      handled = lookupKey("Shift-" + name, keymaps, b => doHandleBinding(cm, b, true))
             || lookupKey(name, keymaps, b => {
                  if (typeof b == "string" ? /^go[A-Z]/.test(b) : b.motion)
                    return doHandleBinding(cm, b);
                });
    } else {
      handled = lookupKey(name, keymaps, b => doHandleBinding(cm, b));
    }
  ...
  function handleCharBinding(cm, e, ch) {
    const handled = lookupKey("'" + ch + "'", allKeyMaps(cm),
  ...
  function onContextMenu(cm, e) {
    ...
    const oldCSS = display.input.style.cssText;
    display.inputDiv.style.position = "absolute";
    display.input.style.cssText = "position: fixed; width: 30px; height: 30px; top: " + (e.clientY - 5) +
      "px; left: " + (e.clientX - 5) + "px; z-index: 1000; background: " +
      (ie ? "rgba(255, 255, 255, .05)" : "transparent") +
      "; outline: none; border-width: 0; outline: none; overflow: hidden; opacity: .05; filter: alpha(opacity=5);";
    if (webkit) const oldScrollY = window.scrollY; // Work around Chrome issue (#2712)
    focusInput(cm);
    ...
    function prepareSelectAllHack() {
      if (display.input.selectionStart != null) {
        ...
        const extval = display.input.value = "\u200b" + (selected ? display.input.value : "");
  ...
  function replaceRange(doc, code, from, to, origin) {
    if (!to) to = from;
    if (cmp(to, from) < 0) { const tmp = to; to = from; from = tmp; }
  ...
  function maybeScrollWindow(cm, coords) {
    ...
    if (doScroll != null && !phantom) {
      const scrollNode = elt("div", "\u200b", null, "position: absolute; top: " +
  ...
  function scrollPosIntoView(cm, pos, end, margin) {
    ...
    for (let limit = 0; limit < 5; limit++) {
      ...
      const scrollPos = calculateScrollPos(cm, Math.min(coords.left, endCoords.left),
  ...
  function scrollIntoView(cm, x1, y1, x2, y2) {
    const scrollPos = calculateScrollPos(cm, x1, y1, x2, y2);
  ...
  function calculateScrollPos(cm, x1, y1, x2, y2) {
    ...
    const screen = display.scroller.clientHeight - scrollerCutOff, result = {};
    if (y2 - y1 > screen) y2 = y1 + screen;
    const docBottom = cm.doc.height + paddingVert(display);
    const atTop = y1 < snapMargin, atBottom = y2 > docBottom - snapMargin;
    if (y1 < screentop) {
      result.scrollTop = atTop ? 0 : y1;
    } else if (y2 > screentop + screen) {
      const newTop = Math.min(y1, (atBottom ? docBottom : y2) - screen);
      if (newTop != screentop) result.scrollTop = newTop;
    }

    const screenleft = cm.curOp && cm.curOp.scrollLeft != null ? cm.curOp.scrollLeft : display.scroller.scrollLeft;
    const screenw = display.scroller.clientWidth - scrollerCutOff - display.gutters.offsetWidth;
    const tooWide = x2 - x1 > screenw;
  ...
  function resolveScrollToPos(cm) {
    ...
    if (range) {
      ...
      const sPos = calculateScrollPos(cm, Math.min(from.left, to.left),
  ...
  function findPosH(doc, pos, dir, unit, visually) {
    ...
    function moveOnce(boundToLine) {
      const next = (visually ? moveVisually : moveLogically)(lineObj, ch, dir, true);
      if (next == null) {
    ...
    if (unit == "char") moveOnce();
      ...
      for (let first = true;; first = false) {
        ...
        const cur = lineObj.text.charAt(ch) || "\n";
    ...
    const result = skipAtomic(doc, Pos(line, ch), origDir, true);
  ...
  CodeMirror.prototype = {
    ...
    getTokenAt: function(pos, precise) {
      const doc = this.doc;
      pos = clipPos(doc, pos);
      const state = getStateBefore(this, pos.line, precise), mode = this.doc.mode;
      const line = getLine(doc, pos.line);
      const stream = new StringStream(line.text, this.options.tabSize);
    ...
    getTokenTypeAt: function(pos) {
      ...
      const cut = type ? type.indexOf("cm-overlay ") : -1;
    ...
    heightAtLine: function(line, mode) {
      ...
      const lineObj = getLine(this.doc, line);
    ...
    addWidget: function(pos, node, scroll, vert, horiz) {
      ...
      if (vert == "over") {
        ...
        const vspace = Math.max(display.wrapper.clientHeight, this.doc.height), hspace = Math.max(display.sizer.clientWidth, display.lineSpace.clientWidth);
    ...
    moveV: methodOp(function(dir, unit) {
      const cm = this, doc = this.doc, goals = [];
      const collapse = !cm.display.shift && !doc.extend && doc.sel.somethingSelected();
      doc.extendSelectionsBy(range => {
        if (collapse)
          return dir < 0 ? range.from() : range.to();
        const headPos = cursorCoords(cm, range.head, "div");
        if (range.goalColumn != null) headPos.left = range.goalColumn;
        goals.push(headPos.left);
        const pos = findPosV(cm, headPos, dir, unit);
    ...
    getScrollInfo: function() {
      const scroller = this.display.scroller, co = scrollerCutOff;
    ...
    scrollIntoView: methodOp(function(range, margin) {
      ...
      if (range.from.line != null) {
        ...
        const sPos = calculateScrollPos(this, Math.min(range.from.left, range.to.left),
  ...
  var commands = CodeMirror.commands = {
    ...
    newlineAndIndent: function(cm) {
      runInOp(cm, () => {
        const len = cm.listSelections().length;
        for (let i = 0; i < len; i++) {
  ...
  var lookupKey = CodeMirror.lookupKey = (name, maps, handle) => {
    function lookup(map) {
      map = getKeyMap(map);
      ...
      for (let i = 0; i < fallthrough.length; ++i) {
        const done = lookup(fallthrough[i]);
        if (done) return done;
      }
      return false;
    }
  ...
  var isModifierKey = CodeMirror.isModifierKey = event => {
  ...
  const TextMarker = CodeMirror.TextMarker = function(doc, type) {
    this.lines = [];
    this.type = type;
    this.doc = doc;
  };
  ...
  TextMarker.prototype.clear = function() {
    ...
    if (cm && this.collapsed && !cm.options.lineWrapping) for (var i = 0; i < this.lines.length; ++i) {
      const visual = visualLine(this.lines[i]), len = lineLength(visual);
  ...
  const LineWidget = CodeMirror.LineWidget = function(cm, node, options) {
    if (options) for (const opt in options) if (options.hasOwnProperty(opt))
  ...
  LineWidget.prototype.clear = function() {
    const cm = this.cm, ws = this.line.widgets, line = this.line, no = lineNo(line);
  ...
  LineWidget.prototype.changed = function() {
    const oldH = this.height, cm = this.cm, line = this.line;
    this.height = null;
  ...
  function addLineWidget(cm, handle, node, options) {
    const widget = new LineWidget(cm, node, options);
    ...
    changeLine(cm.doc, handle, "widget", line => {
      ...
      if (!lineIsHidden(cm.doc, line)) {
        const aboveVisible = heightAtLine(line) < cm.doc.scrollTop;
  ...
  const Line = CodeMirror.Line = function(text, markedSpans, estimateHeight) {
    this.text = text;
  ...
  function runMode(cm, text, mode, state, f, lineClasses, forceToEnd) {
    ...
    const stream = new StringStream(text, cm.options.tabSize);
    let style;
    ...
    while (!stream.eol()) {
      ...
      if (cm.options.addModeClass) {
        const mName = CodeMirror.innerMode(mode, state).mode.name;
    ...
    while (curStart < stream.pos) {
      ...
      const pos = Math.min(stream.pos, curStart + 50000);
      f(pos, curStyle);
      curStart = pos;
    }
  }
  ...
  function getLineStyles(cm, line) {
    if (!line.styles || line.styles[0] != cm.state.modeGen) {
      const result = highlightLine(cm, line, line.stateAfter = getStateBefore(cm, lineNo(line)));
  ...
  function buildLineContent(cm, lineView) {
    ...
    const content = elt("span", null, null, webkit ? "padding-right: .1px" : null);
    const builder = {pre: elt("pre", [content]), content: content, col: 0, pos: 0, cm: cm};
    lineView.measure = {};
  ...
  function buildToken(builder, text, style, startStyle, endStyle, title) {
    if (!text) return;
    const special = builder.cm.options.specialChars;
    let mustWrap = false;
    ...
    if (style || startStyle || endStyle || mustWrap) {
      ...
      const token = elt("span", [content], fullStyle);
  ...
  function buildTokenSplitSpaces(inner) {
    function split(old) {
      let out = " ";
      for (let i = 0; i < old.length - 2; ++i) out += i % 2 ? " " : "\u00a0";
      out += " ";
      return out;
    }
  ...
  function buildCollapsedSpan(builder, size, marker, ignoreWidget) {
    const widget = !ignoreWidget && marker.widgetNode;
    if (widget) {
  ...
  function updateDoc(doc, change, markedSpans, estimateHeight) {
    ...
    const from = change.from, to = change.to, text = change.text;
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    addLineClass: docMethodOp(function(handle, where, cls) {
      return changeLine(this, handle, "class", line => {
        const prop = where == "text" ? "textClass" : where == "background" ? "bgClass" : "wrapClass";
    ...
    removeLineClass: docMethodOp(function(handle, where, cls) {
      return changeLine(this, handle, "class", line => {
        ...
        if (!cur) return false;
          ...
          const found = cur.match(new RegExp("(?:^|\\s+)" + cls + "(?:$|\\s+)"));
    ...
    setBookmark: function(pos, options) {
      const realOpts = {replacedWith: options && (options.nodeType == null ? options.widget : options),
    ...
    copy: function(copyHistory) {
      const doc = new Doc(getLines(this, this.first, this.first + this.size), this.modeOption, this.first);
    ...
    linkedDoc: function(options) {
      ...
      const copy = new Doc(getLines(this, from, to), options.mode || this.modeOption, from);
  ...
  const dontDelegate = "iter insert remove copy getEditor".split(" ");
  for (const prop in Doc.prototype) if (Doc.prototype.hasOwnProperty(prop) && indexOf(dontDelegate, prop) < 0)
    CodeMirror.prototype[prop] = ((method => function() {return method.apply(this.doc, arguments);}))(Doc.prototype[prop]);
  ...
  function lineAtHeight(chunk, h) {
    ...
    outer: do {
      for (var i = 0; i < chunk.children.length; ++i) {
        const child = chunk.children[i], ch = child.height;
  ...
  function copyHistoryArray(events, newGroup, instantiateSel) {
    for (var i = 0, copy = []; i < events.length; ++i) {
      ...
      for (let j = 0; j < changes.length; ++j) {
        ...
        if (newGroup) for (const prop in change) if (m = prop.match(/^spans_(\d+)$/)) {
  ...
  const e_stopPropagation = CodeMirror.e_stopPropagation = e => {
  ...
  function signalLater(emitter, type /*, values...*/) {
    ...
    function bnd(f) {return () => {f.apply(null, args);};}
    for (let i = 0; i < arr.length; ++i)
      list.push(bnd(arr[i]));
  }
  ...
  if ([].indexOf) indexOf = (array, elt) => array.indexOf(elt);
  function map(array, f) {
    const out = [];
    for (let i = 0; i < array.length; i++) out[i] = f(array[i], i);
    return out;
  }
  if ([].map) map = (array, f) => array.map(f);
  ...
  const nonASCIISingleCaseWordChar = /[\u00df\u0590-\u05f4\u0600-\u06ff\u3040-\u309f\u30a0-\u30ff\u3400-\u4db5\u4e00-\u9fcc\uac00-\ud7af]/;
  const isWordCharBasic = CodeMirror.isWordChar = ch => /\w/.test(ch) || ch > "\x80" &&
  ...
  function forEachCodeMirror(f) {
    ...
    const byClass = document.body.getElementsByClassName("CodeMirror");
  ...
  function scrollbarWidth(measure) {
    ...
    const test = elt("div", null, null, "width: 50px; height: 50px; overflow-x: scroll");
  ...
  function lineLeft(line) { const order = getOrder(line); return order ? bidiLeft(order[0]) : 0; }
  ...
  function lineStart(cm, lineN) {
    ...
    const order = getOrder(visual);
    const ch = !order ? 0 : order[0].level % 2 ? lineRight(visual) : lineLeft(visual);
    return Pos(lineN, ch);
  }
  function lineEnd(cm, lineN) {
    ...
    const order = getOrder(line);
    const ch = !order ? line.text.length : order[0].level % 2 ? lineLeft(line) : lineRight(line);
  ...
  var bidiOrdering = ((() => {
    ...
    const arabicTypes = "rrrrrrrrrrrr,rNNmmmmmmrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrmmmmmmmmmmmmmmrrrrrrrnnnnnnnnnn%nnrrrmrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrmmmmmmmmmmmmmmmmmmmNmmmm";
