...
($ => {
	...
	const _re_formatted_numeric = /[',$£€¥%\u2009\u202F]/g;
	
	
	const _empty = d => !d || d === true || d === '-' ? true : false;
	...
	const _numToDecimal = (num, decimalPoint) => {
	...
	const _isNumber = (d, decimalPoint, formatted) => {
	...
	const _isHtml = d => _empty( d ) || typeof d === 'string';
	
	
	const _htmlNumeric = (d, decimalPoint, formatted) => {
		if ( _empty( d ) ) {
			return true;
		}
	...
	const _pluck_order = (a, order, prop, prop2) => {
        const out = [];
        let i=0;
	...
	function _fnColumnOptions( oSettings, iCol, oOptions )
	{
		...
		const attrTest = src => typeof src === 'string' && src.indexOf('@') !== -1;
	...
	function _fnGetObjectDataFn( mSource )
	{
		...
		else if ( mSource === null )
		{
			...
			return data => // type, row and meta also passed, but not used
            data;
		}
	...
	function _fnSetObjectDataFn( mSource )
	{
			      ...
			      mSource.indexOf('[') !== -1 || mSource.indexOf('(') !== -1) )
		{
			...
			return (data, val) => // meta is also passed in, but not used
            setData(data, val, mSource);
		}
		else
		{
			/* Array or flat object mapping */
	...
	{
		const features = settings.oFeatures, sort     = features.bSort, filter   = features.bFilter;
	...
	function _fnBuildAjax( oSettings, data, fn )
	{
		...
		const baseAjax = {
			...
			"error": function (xhr, error, thrown) {
				const log = oSettings.oApi._fnLog;
	...
	function _fnAjaxUpdateDraw ( settings, json )
	{
		...
		// Support both
		const compat = (old, modern) => json[old] !== undefined ? json[old] : json[modern];
	...
	function _fnFilterComplete ( oSettings, oInput, iForce )
	{
		...
		const fnRegex = o => // Backwards compatibility with the bEscapeRegex option
        (o.bEscapeRegex !== undefined ? !o.bEscapeRegex : o.bRegex);
	...
	function _fnInfoMacros ( settings, str )
	{
		...
		// internally
		const formatter  = settings.fnFormatNumber, start      = settings._iDisplayStart+1, len        = settings._iDisplayLength, vis        = settings.fnRecordsDisplay(), all        = len === -1;
	...
	{
		const classes  = settings.oClasses, tableId  = settings.sTableId, menu     = settings.aLengthMenu, d2       = $.isArray( menu[0] ), lengths  = d2 ? menu[0] : menu, language = d2 ? menu[1] : menu;
		...
		$(settings.nTable).bind( 'length.dt.DT', (e, s, len) => {
			if ( settings === s ) {
				$('select', div).val( len );
			}
		} );
	...
	function _fnFeatureHtmlTable ( settings )
	{
		...
		const _div = '<div/>';
		const size = s => !s ? null : _fnStringToCss( s );
	...
	function _fnScrollDraw ( settings )
	{
        ...
        if ( scrollY && scroll.bCollapse ) {
			...
			const iExtra = (scrollX && tableEl.offsetWidth > divBodyEl.offsetWidth) ?
				barWidth :
				0;
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
        ...
        const tableWidthAttr = table.getAttribute('width');
		...
		else
		{
			...
			const tr = tmpTable.find( 'tbody tr' );
	...
	function _fnThrottle( fn, freq ) {
        ...
        return function () {
			const that = this, now  = +new Date(), args = arguments;
	...
	function _fnScrollingWidthAdjust ( settings, n )
	{
		...
		if ( scroll.sX || scroll.sY ) {
			...
			const correction = ! scroll.sX ? scroll.iBarWidth : 0;
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			const rowOne = $('tbody tr:eq(0)', this);
			 ...
			 */
			if ( rowOne.length ) {
				const a = (cell, name) => cell.getAttribute( 'data-'+name ) ? name : null;
	...
	_Api.extend = (scope, obj, ext) => {
        ...
        const methodScoping = (scope, fn, struc) => function () {
	 ...
	 * @ignore
	 */
	const __table_selector = (selector, a) => {
	...
	_api_register( 'page.info()', function ( action ) {
		...
		const settings   = this.context[0], start      = settings._iDisplayStart, len        = settings._iDisplayLength, visRecords = settings.fnRecordsDisplay(), all        = len === -1;
	...
	const __reload = (settings, holdPosition, callback) => {
		if ( _fnDataSource( settings ) == 'ssp' ) {
	...
	const _selector_run = (selector, select) => {
        const out = [];
        let res;
        let a;
        let i;
	...
	const __row_selector = (settings, selector, opts) => _selector_run( selector, sel => {
	...
	_api_registerPlural( 'rows().remove()', 'row().remove()', function () {
		...
		return this.iterator( 'row', (settings, row, thatIdx) => {
			...
			const displayIndex = $.inArray( row, settings.aiDisplay );
	...
	const __details_add = (ctx, row, data, klass) => {
	...
	const __details_remove = (api, idx) => {
	...
	const __details_display = (api, show) => {
		const ctx = api.context;
	...
	const __columnData = (settings, column, r1, r2, rows) => {
		const a = [];
	...
	const __column_selector = (settings, selector, opts) => {
		const columns = settings.aoColumns, names = _pluck( columns, 'sName' ), nodes = _pluck( columns, 'nTh' );
	...
	const __setColumnVis = (settings, column, vis, recalc) => {
        const cols = settings.aoColumns;
	...
	const __cell_selector = (settings, selector, opts) => {
		const data = settings.aoData;
		const rows = _selector_row_indexes( settings, opts );
	...
	_api_register( 'cells()', function ( rowSelector, columnSelector, opts ) {
		...
		const cells = this.iterator( 'table', (settings, idx) => {
			a = [];
	 ...
	 *    } );
	 */
	DataTable.tables = DataTable.fnTables = visible => $.map( DataTable.settings, o => {
	...
	DataTable.models.oSettings = {
		...
		{
			const len      = this._iDisplayLength, start    = this._iDisplayStart, calc     = start + len, records  = this.aiDisplay.length, features = this.oFeatures, paginate = features.bPaginate;
		...
		pageButton: {
			_: function ( settings, host, idx, buttons, page, pages ) {
				...
				try {
					...
					// completed
					const activeEl = $(document.activeElement).data('dt-idx');
	...
	$.extend( DataTable.ext.type.detect, [
		...
		(d, settings) => _empty( d ) || (typeof d === 'string' && d.indexOf('<') !== -1) ?
            'html' : null
	] );
	...
	const __numericReplace = (d, decimalPlace, re1, re2) => {
		if ( d !== 0 && (!d || d === '-') ) {
			return -Infinity;
		}
